...
#     # 1. Sửa lỗi font & ký hiệu (Bổ sung thêm các lỗi thường gặp từ PDF)
#     corrections = {
#         'Ƣ': 'Ư', 'ƣ': 'ư',
#         '±': '±', '–': '-', '—': '-',
#         '': '*',  # Lỗi bullet point
...
#     # Duyệt qua các dòng để xác định Header
#     for line in lines:
#         # Xóa ký tự format cũ (* và #) và khoảng trắng thừa
...
#         # --- CHIẾN LƯỢC ĐÁNH HEADER ---
        
#         # 1. Kiểm tra Header 2 (Số La Mã)
#         if re.match(roman_regex, clean_line):
#             new_lines.append(f"## {clean_line}")
#             h2_roman_found = True
#             h2_count += 1
#             is_header = True
            
#         # 2. Kiểm tra Header 2 (Số thường) - Chỉ khi chưa có La Mã trước đó
#         elif re.match(arabic_regex, clean_line) and not h2_roman_found:
#             new_lines.append(f"## {clean_line}")
#             h2_count += 1
#             is_header = True
            
#         # 3. Kiểm tra Header 1 (Chữ in hoa toàn bộ)
#         elif clean_line.isupper() and len(clean_line) > 3:
#             # Lưu ý: Nếu dòng in hoa cũng là số La Mã/thường thì đã xử lý ở trên
#             new_lines.append(f"# {clean_line}")
#             h1_count += 1
#             is_header = True
